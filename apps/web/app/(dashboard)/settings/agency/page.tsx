...
export default function AgencySettingsPage() {
  const { session, apiFetchWithAuth, refreshAuthSession, updateUser } = useAuth();
  const [feedback, setFeedback] = useState<string | null>(null);
  const [isSaving, startSaving] = useTransition();
  const agency = session?.user.agency ?? null;
  const variant = session?.role === "agency_admin" ? "agency" : "user";
  ...
  return (
    <DashboardShell
      ...
      subtitle="Register the operator account, keep compliance details current, and move the public profile into verified state."
    >
      {/* Status card */}
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <h2 className="mt-1 font-display text-lg text-[var(--color-ink-950)]">
                {agency ? agency.name : "Create your agency profile"}
              </h2>
        ...
        <div className="relative space-y-5">
          ...
          <div className="flex flex-wrap gap-3">
              ...
              onClick={() =>
                startSaving(async () => {
                  try {
                    ...
                    if (!agency) {
                      await apiFetchWithAuth<AgencySummary>("/agencies", {
                        method: "POST",
                        body: JSON.stringify(payload),
                      });
                      const refreshed = await refreshAuthSession();
                      if (refreshed?.user) {
                        updateUser(refreshed.user as UserProfile);
                      }
                      setFeedback("Agency account created. Your session now carries agency access.");
                      return;
                    }
            ...
            >
              ...
              {isSaving ? "Saving..." : agency ? "Save agency details" : "Create agency account"}
            </Button>
